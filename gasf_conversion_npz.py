...
save_dir = "Output_images_npz"  # Directory principale per salvare i dati
...
# Iterazione per salvare le immagini in formato .npz
...
for idx, (row, point) in enumerate(zip(df.iterrows(), points)):
    ...
    gasf_img = X_gasf[valid_idx]  # Immagine GASF già in bianco e nero (valori normalizzati)
    ...
    # Salvataggio dell'immagine come file .npz
    filename = f"sample_{idx+2}.npz"
    filepath = os.path.join(label_dir, filename)
    np.savez_compressed(filepath, gasf_img=gasf_img)
